...
const sendEmail = async ({ to, subject, html }) => {
  // If API keys are missing, fallback to console logging (Great for local testing!)
  if (!process.env.MAILJET_API_KEY || !process.env.MAILJET_SECRET_KEY) {
    console.log('\n──────────────────────────────────────────');
    console.log(`📧  EMAIL NOT SENT (Mailjet API keys not configured)`);
    console.log(`To:      ${to}`);
  ...
  try {
    await mailjet.post('send', { version: 'v3.1' }).request({
      Messages: [
        {
          From: {
            // This MUST be the exact email you verified on your Mailjet account
            Email: process.env.MAILJET_SENDER_EMAIL, 
            Name: "WorkMitra Support"
          },
          To: [
            {
              Email: to,
    ...
    console.log(`Email successfully sent to ${to} via Mailjet`);
  } catch (error) {
...
const sendBanAppealEmailToAdmin = async (worker, message) => {
  // Sends to your configured support email, or falls back to your verified Mailjet sender
  const adminEmail = process.env.SUPPORT_EMAIL || process.env.MAILJET_SENDER_EMAIL || 'support@workmitra.com';
